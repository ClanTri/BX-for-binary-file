...
-- ========= true BSON: { "data": [ { "_id":i32, "name":str, "age":i32 }, ... ] } =========
writeMongoBSON_True :: FilePath -> [(Int32, T.Text, Int32)] -> IO ()
writeMongoBSON_True fp triples = BL.writeFile fp buildTop
  where
    -- cstring: bytes + 0x00
    ...
    bsonStringVal s =
      ...
      in BB.word32LE (fromIntegral n) <> BB.byteString s <> BB.word8 0x00
    ...
    finalizeDoc bodyB =
      ...
      in BB.toLazyByteString (BB.word32LE (fromIntegral total) <> BB.byteString bodyBS <> BB.word8 0x00)

    -- { "_id":i32, "name":str, "age":i32 }
    embedUser :: (Int32, T.Text, Int32) -> BL.ByteString
    embedUser (i, nm, ageM) =
      let body =
             BB.word8 0x10 <> cstringS "_id"  <> BB.word32LE (fromIntegral i) <>
             BB.word8 0x02 <> cstringS "name" <> bsonStringVal (encodeUtf8 nm) <>
             BB.word8 0x10 <> cstringS "age"  <> BB.word32LE (fromIntegral ageM)
      in finalizeDoc body

    -- array value is a *document* whose keys are "0","1","2",...
    buildArrayDoc :: [(Int32, T.Text, Int32)] -> BL.ByteString
    buildArrayDoc xs =
      let body = mconcat
            [ BB.word8 0x03 <> cstringS (show idx)
              <> BB.byteString (BL.toStrict (embedUser x))
            | (idx, x) <- zip [0 :: Int ..] xs
            ]
      in finalizeDoc body

    -- top document with key "data" (type=0x04 array), whose value is the doc above
    buildTop :: BL.ByteString
    buildTop =
      let arrDoc = buildArrayDoc triples
          body   = BB.word8 0x04 <> cstringS "data" <> BB.byteString (BL.toStrict arrDoc)
      in finalizeDoc body
