...
module Sync_Multi_Rearr_test_Switched
  ( 
...
runCLI = do
  ...
  when debug $ do
    ...
    mapM_ (\e -> putStrLn $
          ...
          ", age="        ++ show (userAge e)
          ) (take 5 binEntries)   -- ★只打印前 5 条，防止爆炸

    putStrLn $ "[DEBUG] bson entries: " ++ show (length bsonEntries)
    mapM_ (\e -> putStrLn $
          "  bson -> id=" ++ show (bsonId e) ++
          ", name=" ++ BC.unpack (bsonName e) ++
          ", age="  ++ show (bsonAge e)
          ) (take 5 bsonEntries)  -- ★只打印前 5 条
  ...
  when debug $ putStrLn $ "[DEBUG] common ids:   " ++ show (take 20 ids)
