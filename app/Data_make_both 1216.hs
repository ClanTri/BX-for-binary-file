...
main :: IO ()
main = do
  putStrLn "[LOG] program start" >> hFlush stdout
  ...
  putStrLn $ "[LOG] args = " ++ show args 

  -- ---- MongoDB ----
  putStrLn "[LOG] connecting MongoDB..." >> hFlush stdout
  pipe <- M.connect (M.host "127.0.0.1")
  let mdb   = "test"  :: M.Database
      mcoll = "users" :: M.Collection
  putStrLn "[LOG] MongoDB connected" >> hFlush stdout

  putStrLn "[LOG] inserting Mongo data..." >> hFlush stdout
  ...
  putStrLn "[LOG] Mongo insert done" >> hFlush stdout


  -- ---- PostgreSQL ----
  -- if needed add password=...）
  putStrLn "[LOG] connecting PostgreSQL..." >> hFlush stdout

  let pgConnStr = "host=127.0.0.1 port=5432 dbname=postgres user=postgres password=621307"
  conn <- PG.connectPostgreSQL pgConnStr
  
  putStrLn "[LOG] PostgreSQL connected" >> hFlush stdout
  
  putStrLn "[LOG] inserting Postgres data..." >> hFlush stdout

  (insertedP, usedRows) <- insertPostgres conn tuples
  totalP    <- pgCount conn
  newestP   <- pgNewest conn

  putStrLn "[LOG] Postgres insert done" >> hFlush stdout

  -- ---- Close Connection ----
  
  putStrLn "[LOG] SKIPED closing Mongo..." >> hFlush stdout
  --M.close pipe
  --putStrLn "[LOG] Mongo closed" >> hFlush stdout
  
  putStrLn "[LOG] closing Postgres..." >> hFlush stdout
  PG.close conn
  putStrLn "[LOG] Postgres closed" >> hFlush stdout

  putStrLn "[LOG] exporting files..." >> hFlush stdout
  -- ---- Export File (standard formats) ----

  putStrLn "[LOG] bson written" >> hFlush stdout
  writeMongoBSON_True "mongo_users.bson"      tuples     -- BSON
  putStrLn "[LOG] bson done" >> hFlush stdout

  putStrLn "[LOG] PGCOPY binary written" >> hFlush stdout
  writePostgresCOPY   "postgres_users.bin"     usedRows  -- PGCOPY binary
  putStrLn "[LOG] PGCOPY binary done" >> hFlush stdout

  putStrLn "[LOG] JSON binary written" >> hFlush stdout
  writeMongoJSON      "mongo_users.json"      tuples     -- JSON
  putStrLn "[LOG] JSON binary done" >> hFlush stdout

  putStrLn "[LOG] XML binary written" >> hFlush stdout
  writePostgresXML    "postgres_users.xml"    "postgres_users.xsd" usedRows  -- XML
  putStrLn "[LOG] XML binary done" >> hFlush stdout
