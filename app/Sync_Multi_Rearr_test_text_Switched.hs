-- Version of reducing log

{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE TemplateHaskell #-}
module Sync_Multi_Rearr_test_text_Switched
...
runCLI = do
  ...
  -- 只在 debug 时输出，并且只显示前几条，避免百万行输出拖死
  when debug $ do
    putStrLn $ "[DEBUG] JSON users: " ++ show (length bsonEntries)
    mapM_ (\e -> putStrLn $
          "  json -> id=" ++ show (bsonId e)
          ++ ", name=" ++ bsonName e
          ++ ", age="  ++ show (bsonAge e)
          ) (take 5 bsonEntries)
  ...
  when debug $
    putStrLn $ "[DEBUG] common ids (first 20): " ++ show (take 20 ids)
  ...
  case dir of
    "json2xml" -> do
      ...
      Text.XML.writeFile def "users_synced.xml"
        (assemblePgUsers (if null updatedPg then pgEntries else updatedPg))
    ...
    "xml2json" -> do
      ...
      BL.writeFile "users_synced.json"
        (encode (if null updatedBson then bsonEntries else updatedBson))
