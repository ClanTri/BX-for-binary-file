...
syncAgeField direction binStruct bsonStruct =
    case direction of
      "bin2bson" ->  -- sync to bson
        case put ageSync (age bsonStruct) (age binStruct) of
          Just newAge -> do
              let bsonStruct' = bsonStruct { age = newAge }
              BS.writeFile "users_bson_synced.bson" (assembleBson bsonStruct')
              putStrLn $ "[bin->bson] Synchronized age: " ++ show newAge
          Nothing -> putStrLn "Synchronization failed."
      "bson2bin" ->  -- sync to bin
        case put ageSync (age binStruct) (age bsonStruct) of
          Just newAge -> do
              let binStruct' = binStruct { age = newAge }
              BS.writeFile "users_bin_synced.bin" (assembleBin binStruct')
              putStrLn $ "[bson->bin] Synchronized age: " ++ show newAge
          Nothing -> putStrLn "Synchronization failed."
...
main = do
    ...
    putStrLn "Choose and input：bin2bson or bson2bin"
    direction <- getLine
    syncAgeField direction binStruct bsonStruct