...
--------------------------------------------------------------------------------
-- BSON side (manual parser/writer for { "data": [ { _id, name, age }, ... ] })
--------------------------------------------------------------------------------

parseBsonArray :: BS.ByteString -> [BsonUserEntry]
parseBsonArray bs =
  let afterLen = BS.drop 4 bs
      _typ     = BS.head afterLen        -- 0x04
      rest1    = BS.tail afterLen
      (_, r2)  = BS.break (== 0) rest1   -- "data"
      r3       = BS.drop 1 r2
      arrLen   = bsToWord32LE (BS.take 4 r3)
      arrBody  = BS.take (fromIntegral arrLen - 5) (BS.drop 4 r3)  -- strip trailing 0
  in parseArrayDocs arrBody
...
parseArrayDocs s
  ...
  | BS.head s == 0x00 = []
  | otherwise         =
      let _typ   = BS.head s             -- 0x03
          r1     = BS.tail s
          (_,r2) = BS.break (==0) r1      -- skip "0","1",...
          r3     = BS.drop 1 r2
          dlen   = bsToWord32LE (BS.take 4 r3)
          doc    = BS.take (fromIntegral dlen) r3
          r4     = BS.drop (fromIntegral dlen) r3
      in parseBsonUserEntry doc : parseArrayDocs r4

parseBsonUserEntry :: BS.ByteString -> BsonUserEntry
parseBsonUserEntry doc =
  let body           = BS.drop 4 doc
...
parseFieldInt32 s expect =
  let typ     = BS.head s
      _       = if typ /= 0x10 then error "Expected int32" else ()
      r1      = BS.tail s
      (nm,r2) = BS.break (==0) r1
      fname   = BC.unpack nm
      r3      = BS.drop 1 r2
      val     = bsToWord32LE (BS.take 4 r3)
      r4      = BS.drop 4 r3
  in if fname == expect then (val, r4) else error $ "Expected " ++ expect ++ ", got " ++ fname
...
assembleBsonFile es =
  let arrBody = BS.concat (zipWith assembleBsonArrayItem [0..] es) `BS.snoc` 0x00
      arrLen  = encodeWord32LE (fromIntegral $ BS.length arrBody + 4)
      arrDoc  = BS.concat [arrLen, arrBody]                           -- array is a document
      docBody = BS.concat [BS.singleton 0x04, "data", BS.singleton 0x00, arrDoc] `BS.snoc` 0x00
      docLen  = encodeWord32LE (fromIntegral $ BS.length docBody + 4)
  in BS.concat [docLen, docBody]

assembleBsonArrayItem :: Int -> BsonUserEntry -> BS.ByteString
assembleBsonArrayItem idx (BsonUserEntry bid bage bname) =
  let docBody = BS.concat
...
-- ====== Parse custom PUSR / MUSR formats ======
-- PUSR: "PUSR" | ver i32BE | count i32BE | count * [ id i32BE | name u16BE+utf8 | gender u16BE+utf8 | age i32BE ]
-- MUSR: "MUSR" | ver i32BE | count i32BE | count * [ id i32BE | name u16BE+utf8 | age i32BE ]

parseText16BE :: BS.ByteString -> (BS.ByteString, BS.ByteString)
parseText16BE s =
  let len16 = bsToWord16BE (BS.take 2 s)
      body  = BS.take (fromIntegral len16) (BS.drop 2 s)
      rest  = BS.drop (2 + fromIntegral len16) s
  in (body, rest)

takeN :: Int -> BS.ByteString -> (BS.ByteString, BS.ByteString)
takeN n s = (BS.take n s, BS.drop n s)

-- Big-endian Int32 読み（Word32 -> Int に丸め）
readI32BE :: BS.ByteString -> (Int, BS.ByteString)
readI32BE s =
  let (wbs, rest) = takeN 4 s
      w           = bsToWord32BE wbs
  in (fromIntegral w :: Int, rest)

readW32BE :: BS.ByteString -> (Word32, BS.ByteString)
readW32BE s =
  let (wbs, rest) = takeN 4 s
  in (bsToWord32BE wbs, rest)

-- PUSR
parsePUSR :: BS.ByteString -> [PgUserEntry]
parsePUSR bs0 =
  let (magic, b1) = takeN 4 bs0
  in if magic /= "PUSR"
       then error "[PUSR] bad magic; expected 'PUSR'"
       else
         let (_ver, b2)   = readI32BE b1
             (cnt,  b3)   = readI32BE b2
         in parsePUSRRows cnt b3
  where
    parsePUSRRows 0 s = []
    parsePUSRRows k s =
      let (i32, s1)     = readW32BE s
          (nm,  s2)     = parseText16BE s1
          (gd,  s3)     = parseText16BE s2
          (ageW, s4)    = readW32BE s3
          entry         = PgUserEntry (fromIntegral i32) nm gd ageW
      in entry : parsePUSRRows (k-1) s4

-- MUSR
parseMUSR :: BS.ByteString -> [BsonUserEntry]
parseMUSR bs0 =
  let (magic, b1) = takeN 4 bs0
  in if magic /= "MUSR"
       then error "[MUSR] bad magic; expected 'MUSR'"
       else
         let (_ver, b2) = readI32BE b1
             (cnt, b3)  = readI32BE b2
         in parseMUSRRows cnt b3
  where
    parseMUSRRows 0 s = []
    parseMUSRRows k s =
      let (i32, s1)   = readW32BE s
          (nm,  s2)   = parseText16BE s1
          (ageW, s3)  = readW32BE s2
          entry       = BsonUserEntry (fromIntegral i32) ageW nm
      in entry : parseMUSRRows (k-1) s3
...
runCLI = do
  ...
  if null ids
    then do
      ...
      putStrLn "  - For bson: does the top-level actually contain {\"data\": [...]}?"
