...
parsePgTextField s =
  let len     = bsToWord32BE (BS.take 4 s)
      payload = BS.take (fromIntegral len) (BS.drop 4 s)
      rest    = BS.drop (4 + fromIntegral len) s
  in (payload, rest)

parsePgIntField :: BS.ByteString -> (Word32, BS.ByteString)
parsePgIntField s =
  let _len = bsToWord32BE (BS.take 4 s)                 -- typically 4
      val  = bsToWord32BE (BS.take 4 (BS.drop 4 s))
      rest = BS.drop 8 s
  in (val, rest)
...
assembleBinFile es =
  let header = BS.pack ([80,71,67,79,80,89,10,255,13,10,0,0,0,0,0,0,0,0,0] :: [Word8]) -- "PGCOPY\n\377\r\n" + zeros
...
main = do
  ...
  binRaw  <- BS.readFile "user.bin"
  bsonRaw <- BS.readFile "user.bson"
